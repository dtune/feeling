...
public class DataLayoutBeanFactory extends DefaultBeanFactory {
     ...
     * <b>{@link InputDataCategory}</b> for this implementation.
     *
    ...
    {
        return InputDataCategory.class;
    }
	...
	public  void initBean(Object bean, BeanDeclaration data) throws Exception
	{
		...
		if (nestedBeans != null) {
		    for (Iterator it = nestedBeans.entrySet().iterator(); it.hasNext();) {
		        ...
		        //キーの場合、keyのlayoutを１ずつ初期化する
		        if (propName.equals(DataLayoutDeclaration.LAYOUT_DATA)) {
		        	for (Object obj : (List)e.getValue()) {
				        ...
				        DataLayout keyLayout = (DataLayout) createBean(clz, decl, null);
				        ((InputDataCategory)bean).getLayoutData().add(keyLayout);
		        	}
		        } else {
