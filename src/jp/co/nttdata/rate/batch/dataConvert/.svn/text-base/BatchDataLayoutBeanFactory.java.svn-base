...
public class BatchDataLayoutBeanFactory extends DefaultBeanFactory {
	...
	public  void initBean(Object bean, BeanDeclaration data) throws Exception
	{
		...
		if (nestedBeans != null) {
		    for (Iterator it = nestedBeans.entrySet().iterator(); it.hasNext();) {
		        ...
		        //ÉLÅ[ÇÃèÍçáÅAkeyÇÃlayoutÇÇPÇ∏Ç¬èâä˙âªÇ∑ÇÈ
		        if (propName.equals(BatchDataLayoutDeclaration.LAYOUT_DATA)) {
		        	for (Object obj : (List)e.getValue()) {
				        ...
				        RateKeyLayout keyLayout = (RateKeyLayout) createBean(clz, decl, null);
				        ((BatchDataLayout)bean).getLayoutData().add(keyLayout);
		        	}
		        } else {
